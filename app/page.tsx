...
export default function Home() {
  ...
  useEffect(() => {
    const saved = localStorage.getItem("runningAppData");
    if (saved) {
      try {
  ...
  useEffect(() => {
    if (paceData) {
      localStorage.setItem("runningAppData", JSON.stringify(paceData));
    }
  }, [paceData]);
  ...
  const handleReset = () => {
    setPaceData(null);
    localStorage.removeItem("runningAppData");
  };
      ...
      <header className="border-b border-white/10 bg-gradient-to-r from-red-600/10 to-pink-500/10 backdrop-blur-sm">
        <div className="max-w-7xl mx-auto px-6 sm:px-8 lg:px-12 py-6">
          <div className="flex items-center gap-3">
            <div className="p-2 bg-gradient-to-br from-red-600 to-pink-500 rounded-xl"></div>
            <div>
          ...
          <div className="space-y-6">
            {paceData ? (
              ...
              <div className="bg-[#1a1a1a] border border-white/10 rounded-2xl p-12 text-center">
                <div className="w-16 h-16 bg-gradient-to-br from-red-600/20 to-pink-500/20 rounded-full mx-auto mb-4 flex items-center justify-center">
